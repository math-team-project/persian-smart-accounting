...
### هفته ۱۵ — فاز داشبورد: ورود کاربر، پروژه‌ها و تاریخچهٔ اجراها

**کارهای انجام‌شده:**
- افزودن **لایهٔ ورود و پروژه** روی دو کارگاه موجود: ورود با کوکی نشست (هش رمز با
  `passlib`)، ساخت نخستین کاربر از `/setup` یا با `python -m api.cli create-user`، و
  جداسازی کامل داده‌های هر کاربر.
- **رجیستری کارگاه‌ها** (`api/workshops/registry.py`): «چه کارگاه‌هایی وجود دارد»
  به داده تبدیل شد؛ کارت‌های صفحهٔ پروژه، ناوبری، ثبت روترها در FastAPI و مسیر
  `/projects/{id}/{slug}` همه از همین رجیستری ساخته می‌شوند. کارگاه سوم فقط با یک
  ماژول جدید + یک خط ثبت اضافه می‌شود (بدون تغییر قالب‌ها).
- **جلوگیری از تداخل اجراهای هم‌زمان**: هر job با `(project_id, workshop_type, run_id)`
  برچسب می‌خورد و وضعیتش سمت سرور (نه در حافظهٔ مرورگر) می‌ماند؛ پنل «کارهای در
  جریان» صفحهٔ پروژه وضعیت زندهٔ چند اجرای هم‌زمان را با هم نشان می‌دهد.
- **تاریخچهٔ نتیجه‌محور**: هنگام پایان هر اجرا یک callback (بدون هیچ تغییری در
  `pipeline.py`/`audit_pipeline.py`) فایل نتیجه را در `data/results/project-N/` ذخیره
  و ردیف `workshop_runs` را ثبت می‌کند. ردیف تاریخچه فقط متادیتای نتیجه را نگه
  می‌دارد؛ فایل‌های ورودی مثل قبل پس از پردازش حذف می‌شوند.
- **حذف کامل پروژه**: حذف ردیف‌های پایگاه‌داده (پروژه/اجراها/تنظیمات)، حذف job های
  در حافظه و پاک‌سازی پوشهٔ نتیجه‌های همان پروژه روی دیسک -- با یک مرحلهٔ تأیید صریح
  در رابط کاربری.
- پایگاه‌دادهٔ SQLite روی SQLAlchemy با لایهٔ repository (جدا از PostgreSQL قدیمی
  `db_management/` که دست‌نخورده ماند)، فایل‌های نتیجه روی دیسک، و نمایش تاریخ شمسی
  و ارقام فارسی در همهٔ صفحه‌های جدید.
- بازنویسی تست‌های لایهٔ API (۴۸ تست): ورود/راه‌اندازی، پروژه‌ها و جداسازی بین
  کاربران، جریان کامل هر دو کارگاه با ثبت تاریخچه، نبودِ دادهٔ ورودی در تاریخچه،
  اجراهای هم‌زمان، و صحت حذف بدون باقی‌ماندن داده/فایل یتیم.
- تأیید دستی end-to-end با داده‌های واقعی پروژه: اجرای واقعی چک‌لیست روی فایل‌های
  بودجه/صورت‌های مالی/تراز سال ۹۸ (۴۲ پرسش، تولید گزارش کمیسیون)، اجرای هم‌زمان
  چک‌لیست و خلاصه‌سازی در یک پروژه، دانلود نتیجه، و حذف پروژه بدون هیچ دادهٔ یتیم.

**چالش‌ها:**
- خطای «اعتبار ناکافی» سرویس‌های هوش مصنوعی (کلیدهای موجود در `.env` موجودی ندارند)
  اجرای واقعی خلاصه‌سازی و تولید گزارش کمیسیون را در آزمایش نهایی ناموفق کرد؛ مسیر
  خطا به‌درستی مدیریت و در تاریخچه ثبت شد (اجرای چک‌لیست موفق + ردیف ناموفق
  خلاصه‌سازی)، اما برای تأیید نهاییِ تولید فایل گزارش، کلید دارای اعتبار لازم است.
- تفاوت ساعت ذخیره‌شده (UTC) با ساعت محلی در SQLite: مقدار naive برگشتی باید UTC در
  نظر گرفته شود (در `api/utils/formatting.py` اصلاح و تست شد).

---

### هفته ۱۶ — بازه‌ی ۱۴۰۵/۰۵/۲۴ تا ۱۴۰۵/۰۶/۲۸: پایگاه‌دانش (RAG)، چت‌بات مالی و حذف کامل پروژه

**کارهای انجام‌شده:**
- **پایگاه‌دانش (Knowledge Base) به‌عنوان یک موجودیت واقعی:** دو جدول تازه
  (`knowledge_bases` و `kb_files`) و اشاره‌گر `projects.latest_ready_kb_id` به‌عنوان
  «تنها منبع حقیقت» برای «پایگاه‌دانش فعلی این پروژه». هر اجرای چک‌لیست یک نسخه‌ی
  تازه با `kb_id` مستقل (UUID) می‌سازد و خودِ `rag_chat_module`
  (`llm_variable_resolver`) **دست‌نخورده** ماند؛ داشبورد فقط از طریق
  `api/services/rag_ai_adapter.py` به آن وصل می‌شود.
- **وصل‌شدن RAG به کارگاه چک‌لیست** (`api/services/checklist_kb_service.py`):
  ایندکس‌سازی هم‌زمان با خودِ ارزیابی چک‌لیست در یک ترد جدا شروع می‌شود؛ یک نقطه‌ی
  «join» سبک (`ChecklistKBJoin` با `threading.Event`، همان الگوی هماهنگی موجود پروژه)
  پیش از تولید گزارش کمیسیون صبر می‌کند تا ایندکس آماده شود، سپس موارد FALSE خام با
  تطبیق هوشمند روی همان پایگاه‌دانش بازبینی می‌شوند و نتیجه‌ی اصلاح‌شده **جایگزین**
  موارد خام در گزارش می‌شود. هر خطای این مسیر graceful مدیریت می‌شود و در بدترین
  حالت گزارش از موارد خام ساخته می‌شود (کارگاه چک‌لیست هرگز نمی‌شکند).
- **بازخورد نتیجه به پایگاه‌دانش:** نتیجه‌ی تطبیق‌شده به‌صورت یک سند Markdown به همان
  پایگاه‌دانش اضافه می‌شود، بعد پایگاه‌دانش `ready` و اشاره‌گر پروژه به‌روزرسانی
  می‌شود؛ سپس سیاست نگهداری (`PSA_KB_RETENTION_COUNT`) نسخه‌های قدیمی‌تر همان پروژه را
  از دیسک و پایگاه‌داده هرس می‌کند.
- **کارگاه «چت‌بات مالی» (چهارمین کارگاه رجیستری):** یک کارگاه بدون job که گفتگوها را
  فقط به‌صورت **متادیتا** نگه می‌دارد (`chat_sessions`: عنوان + زمان؛ هیچ متن پیامی
  نه در پایگاه‌داده و نه روی دیسک). صفحه در نبود پایگاه‌دانش آماده **دروازه‌بندی**
  می‌شود و `/ask` با ۴۰۹ و پیام فارسی واضح رد می‌شود. زمینه‌ی چندنوبتی صرفاً به ابتدای
  همان پرسش اضافه می‌شود (سرور کاملاً بی‌حالت می‌ماند، چون `chat_ask` پکیج دست‌نخورده
  است)، و پاسخ‌ها به‌صورت Markdown سمت مرورگر و بدون هیچ کتابخانه‌ی خارجی رندر
  می‌شوند (خروجی مدل همیشه اول escape می‌شود).
- **`chunks.jsonl` به‌عنوان مکمل اجباری Chroma:** کش متن `ChromaVectorStore` فقط
  درون-فرایندی است و در Chroma هیچ `documents`ی ذخیره نمی‌شود؛ یعنی در یک فرایند
  تازه (درخواست چت‌بات) چیزی برای جست‌وجو وجود نداشت. متن هر chunk کنار همان
  پایگاه‌دانش ذخیره و در مسیر فقط-خواندنی بازسازی می‌شود
  (`rag_ai_adapter.build_readonly_knowledge_base`).
- **حذف کامل پروژه به‌صورت آبشاری و امن:** ترتیب حذف بر پایه‌ی ایمنی چیده شد -- ابتدا
  رد کردن درخواست اگر پروژه job فعالی داشته باشد (خطای فارسی + ۴۰۹)، سپس job های
  در حافظه، ردیف‌های گفتگو، پوشه‌های دیسکی پایگاه‌دانش (دقیقاً بر اساس ردیف‌های همان
  پروژه، **نه** با glob زدن روی دیسک)، سپس ردیف‌های `kb_files`/`knowledge_bases` (با
  پاک‌کردن اشاره‌گر FK پیش از حذف) و در پایان ردیف پروژه و فایل‌های نتیجه. سه تابع
  حذف تازه همگی صریحاً `project_id` (و `user_id` برای ساخت مسیر) می‌گیرند و از هیچ
  مسیری روی HTTP قابل‌دسترس نیستند (با تست خودکار روی `app.openapi()` قفل شد).
- **رفع یک خطای واقعی هم‌زمانی (TOCTOU) در سیاست نگهداری:** چون نشست با
  `expire_on_commit=False` ساخته می‌شود، «کدام نسخه‌ها زائدند» و «کدام نسخه فعلی
  است» اگر دو خواندن جدا بودند می‌توانستند بین دو اجرای هم‌زمان ناسازگار شوند و
  پایگاه‌دانشِ در حال استفاده حذف شود. حالا خودِ پرس‌وجو اشاره‌گر فعلی را در همان
  دستور SQL کنار می‌گذارد (اتمیک) و بررسی حلقه فقط لایه‌ی دفاعی دوم است.
- **تست‌ها:** افزودن تست‌های جداسازی/هم‌زمانی (دو اجرای هم‌زمان در یک پروژه، اجرای A
  + پرسش B، مسابقه‌ی نگهداری با پرسش هم‌زمان)، تست آبشار حذف پروژه (شامل «حذف
  پروژه‌ی A هرگز به پروژه‌ی B دست نمی‌زند»)، تست دروازه‌بندی چت‌بات، تست افتادن
  embedder روی CPU وقتی `torch.cuda.is_available()` مقدار `False` می‌دهد، و یک
  **آزمون دود سرتاسری** که کل مسیر «آپلود → چک‌لیست → ایندکس → تطبیق → گزارش →
  چت‌بات → نگهداری → حذف پروژه» را با جعلی‌سازی فقط مرز مدل/embedder/استور می‌گذراند.
  `tests/conftest.py` هم اکنون `PSA_VECTOR_STORE_ROOT` را به پوشه‌ی موقت هدایت می‌کند
  (پیش‌تر این یک شکافِ جداسازی تست بود: پاک‌سازی پوشه‌ها می‌توانست به `data/` واقعی
  برسد).
- **تست کامل بدون شبکه/GPU:** `pytest -q` با بیش از ۲۵۰ تست سبز است و هیچ‌کدام به
  LLM، مدل embedding، Chroma، Tesseract یا LibreOffice واقعی نیاز ندارند.

**چالش‌ها:**
- **کش درون‌حافظه‌ای `rag_chat_module`:** چون آن پکیج طبق طراحی تغییر نمی‌کند و متن
  جدول‌ها را فقط در حافظه نگه می‌دارد، مسیر فقط-خواندنی بدون یک مکمل روی دیسک کار
  نمی‌کرد؛ راه‌حل، sidecar `chunks.jsonl` در خود داشبورد بود (بدون هیچ تغییری در پکیج).
- **ابهام ترتیب «آماده‌شدن» و «ساخته‌شدن» پایگاه‌دانش:** ترتیب `created_at` با ترتیب
  ready شدن یکی نیست؛ همین باعث خطای TOCTOU بالا شد و با کنارگذاشتن اتمیک اشاره‌گر
  در خود پرس‌وجو رفع شد.
- **نبودِ تضمین قابل‌اتکای `ON DELETE CASCADE` در SQLite:** مثل قبل، ردیف‌های وابسته
  صریحاً (و با ترتیب FK-محترم) حذف می‌شوند تا هرگز ردیف یتیم نماند.

**تصمیمات:**
- حذف پروژه یک عملیات **غیرقابل‌بازگشت** است؛ به همین دلیل همه‌ی توابع حذف تازه
  نام‌های کامل و صریح دارند (`delete_all_knowledge_bases_for_project`،
  `delete_vector_store_directories_for_project`،
  `delete_all_chat_sessions_for_project`) و هیچ‌کدام دامنه‌ی «همه‌ی پروژه‌ها» ندارند.
- حذف روی دیسک **پیش از** حذف ردیف‌های پایگاه‌داده انجام می‌شود (فهرست دقیق مسیرها از
  ردیف‌های پروژه-محور می‌آید)، و پوشه‌ی والد پروژه فقط اگر خالی باشد حذف می‌شود؛ در
  غیر این صورت دست‌نخورده می‌ماند و فقط هشدار لاگ می‌شود.
- چت‌بات عمداً **بی‌حالت (stateless)** است: زمینه‌ی گفتگو فقط در همان درخواست زندگی
  می‌کند و در حافظه‌ی مرورگر نگه داشته می‌شود؛ بنابراین سرور هیچ تاریخچه‌ای را
  نمی‌نویسد.
