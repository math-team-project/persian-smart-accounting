...
# هزار نویسه است. مقدار پیش‌فرض همان مقدار جاافتاده‌ی ماژول‌های موجود پروژه
# (``audit_report_generator.config.LLMConfig``) است که با provider فعلی اثبات شده؛
# سقف تعداد موارد جزئی هر محور (``BudgetConfig.max_detail_findings_per_axis``) هم
# برای همین منظور وجود دارد تا خروجی از این سهمیه بیرون نزند. اگر provider/مدل
# دیگری سهمیه‌ی کمتری داشته باشد، فاز تنظیمات می‌تواند آن را کم کند.
