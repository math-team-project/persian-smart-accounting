...
# so `import readers` never fails on a machine missing pytesseract or
# pdf2image - those are only needed when an OCR fallback actually runs.
...
def _pdf_to_text(pdf_path: str) -> str:
    """Convert a Persian PDF to text via OCR (page by page)."""
    import pytesseract
    from pdf2image import convert_from_path

    # Optional Windows tesseract path override (env-driven, non-fatal).
    default_win = r"C:/Program Files/Tesseract-OCR/tesseract.exe"
    tesseract_cmd = os.environ.get("TESSERACT_CMD") or (
        default_win if os.path.exists(default_win) else None
    )
    if tesseract_cmd:
...
def extract_text(file_path: str | Path) -> ExtractedDocument:
    ...
    if suffix == ".pdf":
        # The original implementation renamed the input file to "Report.pdf"
        # in place. That destroys the caller's filename and can clobber an
        # existing Report.pdf in the same directory. Instead, copy to a
        # temp path with a short ASCII name - some OCR backends still choke
        # on long Persian filenames or non-ASCII paths.
        with tempfile.TemporaryDirectory() as tmp_dir:
            tmp_pdf = Path(tmp_dir) / "Report.pdf"
            shutil.copy2(path, tmp_pdf)
            ocr_text = _pdf_to_text(str(tmp_pdf))
        if not ocr_text.strip():
            raise ExtractionError(
                "هیچ متنی از فایل PDF استخراج نشد (OCR متن خالی برگرداند)."
            )
        return ExtractedDocument(source_path=path, text=ocr_text)
    ...
    raise ExtractionError(
        ...
        f"فرمت‌های مجاز: .pdf, .doc, .docx, .rtf, .odt"
    )
